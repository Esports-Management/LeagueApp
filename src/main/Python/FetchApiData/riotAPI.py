...
class Summoner:
    ...
    def get_challenger_by_queue_ranked_solo_5x5(self, lol_watcher, region):
        self.queue.update({"RANKED_SOLO_5x5": lol_watcher.league.challenger_by_queue(region=region, queue="RANKED_SOLO_5x5")})

    def get_challenger_by_queue_ranked_flex_sr(self, lol_watcher, region):
        self.queue.update({"RANKED_FLEX_SR": lol_watcher.league.challenger_by_queue(region=region, queue="RANKED_FLEX_SR")})
    ...
    def get_match_info_by_match_id(self, lol_watcher, region, match_id):
        ...
        for i in range(len(match_metadata['info']['participants'])):
            ...
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['kills'] = match_metadata['info']['participants'][i]['kills']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['deaths'] = match_metadata['info']['participants'][i]['deaths']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['assists'] = match_metadata['info']['participants'][i]['assists']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['firstBloodKill'] = match_metadata['info']['participants'][i]['firstBloodKill']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['goldEarned'] = match_metadata['info']['participants'][i]['goldEarned']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['pentaKills'] = match_metadata['info']['participants'][i]['pentaKills']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['timeCCingOthers'] = match_metadata['info']['participants'][i]['timeCCingOthers']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['totalTimeCCDealt'] = match_metadata['info']['participants'][i]['totalTimeCCDealt']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['totalDamageDealtToChampions'] = match_metadata['info']['participants'][i]['totalDamageDealtToChampions']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['totalMinionsKilled'] = match_metadata['info']['participants'][i]['totalMinionsKilled']
            match_statistics[match_metadata['info']['participants'][i]['summonerName']]['visionScore'] = match_metadata['info']['participants'][i]['visionScore']
